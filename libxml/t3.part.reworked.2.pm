...
our $xpath_schemaloc_ns   = XML::LibXML::XPathExpression->new('//@xsi:schemaLocation');
our $xpath_schemaloc_nons = XML::LibXML::XPathExpression->new('//@xsi:noNamespaceSchemaLocation');
...
sub main {
    ...
    try {
       ...
       foreach my $node ($xpc->findnodes($xpath_schemaloc_ns)) {
   warn "-----looping ---1----\n";
           my ($schema_ns, $schema_location) = split(' ', $node->to_literal());
   warn "-----looping ---2----\n";
          warn Dumper $node;
         # _validate_xmlnode_vs_its_schema ($node, $schema_location);
           _validate_xmlnode_vs_its_schema ($dom, $schema_location);
   warn "-----looping ---3----\n";
       }
    }
    catch {
